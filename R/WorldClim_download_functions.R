...
WorldClim.HistClim.download <- function(save.location = "./",
  ...
  for(vers in version.var){
    URL.1 <- switch (vers,
                     "1.4" = "http://biogeo.ucdavis.edu/data/climate/worldclim/1_4/grid/cur/",
                     "2.1" = "http://biogeo.ucdavis.edu/data/worldclim/v2.1/base/wc2.1_",
                     stop()
    )
          ...
          if(parm.temp != "bio"){
            ...
            for (month in month.var) {
              if(parm.temp != "prec"){
                ...
                br <- crop(raster.temp, extent(0, 180, -90, 0))
                # recalculate like:
        ...
        for(rcp in emission.scenario.var){
          ...
          for(year in time.interval.var) {
            ...
            if(parm != "bio"){
              ...
              # values(r) <- as.numeric(values(r)/10)
            }
            # Hier weiter mit alle-betreffenden Bearbeitungen
            # print(list.files(temp.temp.save.location, pattern = ".tif", full.names = FALSE))
            # if clipping is TRUE ...
