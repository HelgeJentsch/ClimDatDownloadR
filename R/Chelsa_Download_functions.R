...
Chelsa.Clim.download <- function(save.location = "./",
  ...
  for(i in parameter){
    ...
    interm <- switch(i,
                     "prec" = "prec/",
                     "temp" = "temp/integer/temp/",
                     "tmax" = "temp/integer/tmax/",
                     "tmin" = "temp/integer/tmin/",
                     "bio"  = "bioclim/integer/",
                     stop())
      ...
      if(i != "bio10"){
        ...
        for(month in 1:length(month.var)){
          ...
          if(!file.exists(dest.temp)){
            ...
            URL.temp <-
              paste0("ftp://envidatrepo.wsl.ch/uploads/chelsa/chelsa_V1/climatologies/",
        ...
        for(bio in bio.var){
          ...
          URL.temp <-
            paste0("ftp://envidatrepo.wsl.ch/uploads/chelsa/chelsa_V1/",
            ...
            if(i != "bio"){
              for(month in month.var){
                dest.temp <- paste0(temp.temp.save.location, "CHELSA_", model,
                                    ...
                                    "_", time.interval, ".tif")
                if(!file.exists(dest.temp)){
                  URL.temp <- paste0("https://envidatrepo.wsl.ch/uploads/chelsa/chelsa_V1/",
...
Chelsa.lgm.download <- function(save.location = "./",
      ...
      if(i != "bio"){
        ...
        for(month in 1:length(month.var)){
          ...
          if(!file.exists(dest.temp)){
            if(i != "temp") {
              URL.temp <-
                paste0("ftp://envidatrepo.wsl.ch//uploads/chelsa/chelsa_V1/pmip3/",
            ...
            if(url.exists(URL.temp)){
              ...
              if(i != "prec"){
                ...
                raster.temp <- raster.temp-273.15
  ...
  if(download.dem == TRUE){
    if(!file.exists(paste0(temp.temp.save.location, "CHELSA_PMIP_dem_global.tif"))){
      download.file(url = "https://envidatrepo.wsl.ch/uploads/chelsa/chelsa_V1/pmip3/DEM/high_longlat.tif",
                    destfile = paste0(temp.temp.save.location, "CHELSA_PMIP_dem_global.tif"),
                    overwrite = TRUE,
                    mode = 'wb',
                    quiet = FALSE)
    }
...
Chelsa.CRUts.download <- function(save.location = "./",
  ...
  for(i in parameter){
    ...
    for (year_month in ts_string){
      ...
      rm(zero)
      URL.temp <-
        paste0("https://envidatrepo.wsl.ch/uploads/chelsa/chelsa_V1/timeseries20c/",
