...
clipping.tif  <- function(clip.save.location = "./",
  ...
  # requireNamespace("sp")
  global.crs <- crs(projection("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))
  if(length(clip.extent)!=4 & !is.null(clip.extent)) stop()
  if(is.null(clip.shapefile) & is.null(clip.extent)) stop()
  ...
  if(!is.null(clip.shapefile)){
    ...
    if(temp.shp.crs != "GCS"){
      warning("Shapefile is not in GCS! It will be transformed in the next step.", immediate. = T)
...
stacking.downloaded.data <- function(stack.save.location = "./",
  ...
  # print(temp.list.file.names)


  # stack.save.location <- temp.temp.save.location
  ...
  for (directory in stack.save.location) {
    temp.stack.files <- list.files(directory, full.names = TRUE,
                                   recursive = FALSE,
                                   pattern = ".tif")
    if(length(temp.stack.files)==0){
      temp.stack.files <- list.files(directory, full.names = TRUE,
                                     recursive = FALSE,
                                     pattern = ".asc")
      if(length(temp.stack.files)==0) stop()
    }
    temp.stack.file.names <- list.files(directory,
                                        full.names = FALSE,
                                        recursive = FALSE,
                                        pattern = ".tif")
    ...
    for (layer in 1:length(temp.stack.files)){
      if(layer != 1){
        ...
        temp.raster <- raster(temp.stack.files[layer], head = temp.stack.file.names[layer])
        temp.raster <- flip(temp.raster, direction = 2)

        # temp.raster.data <- rotate(matrix(as.numeric(values(temp.raster)),
        #                                   nrow = nlat,
        #                                   ncol = nlon,
        #                                   byrow = TRUE))

        temp.raster.data <- as.matrix(data.frame(values(temp.raster)))
        temp.raster.data <- array(temp.raster.data,
                                  dim=c(openNC$var$Variable$dim[[1]]$len,
        ...
        time <- variable.numbers
        ntime <- length(time)

        # was: rotate(matrix(as.integer(values(temp.raster)),
        # temp.raster.data <- rotate(raster(matrix(as.numeric(values(temp.raster)),
        #                                   nrow = nlat,
        #                                   ncol = nlon,
        #                                   byrow = TRUE)))
        temp.raster.data <- as.matrix(data.frame(values(temp.raster)))
        dim(temp.raster.data)
        temp.raster.data <- array(temp.raster.data, dim=c(nlon,nlat,ntime))
        ...
        timedim <- ncdim_def('time',"m", vals = as.double(time), unlim = T)
        persdim <-

          # define variables
          fillvalue <- -9999
        # PREC_dlname <- 'precipitation'
        prec_def <- ncvar_def(name = 'Variable',
                              units = 'various',
                              dim = list(londim,latdim,timedim),
                              missval = fillvalue,
                              # longname = PREC_dlname,
                              prec = 'single')
        ...
        ncatt_put(openNC,"lon","axis","X") #,verbose=FALSE) #,definemode=FALSE)
        ncatt_put(openNC,"lat","axis","Y")
        ncatt_put(openNC,"time","axis","T")
